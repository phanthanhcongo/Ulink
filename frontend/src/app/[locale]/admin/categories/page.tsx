...
export default async function AdminCategoriesPage({ params }: PageProps) {
  ...
  try {
    ...
    // 2. Fetch categories (non-archived)
    const res = await client.request(
      readItems('product_categories', {
        filter: { status: { _in: ['published', 'draft'] } },
        fields: ['id', 'name', 'slug', 'description', 'parent.id', 'parent.name', 'status'],
        sort: ['sort', 'name'],
        limit: -1
      } as any)
    );
    ...
    console.error('Failed to load categories in admin dashboard:', err);
    try {
  ...
  return <CategoriesClient initialCategories={categories} error={error} />;
}
