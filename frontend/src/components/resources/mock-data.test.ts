...
test('resources mock data keeps items per visible group', () => {
  const counts = MOCK_RESOURCES.reduce<Record<string, number>>((acc, item) => {
    acc[item.category] = (acc[item.category] ?? 0) + 1;
    return acc;
  }, {});

  assert.equal(MOCK_RESOURCES.length, 12);
  assert.equal(counts.guide, 6);
  assert.equal(counts.standard, 1);
  assert.equal(counts['case-study'], 2);
  assert.equal(counts.news, 2);
  assert.equal(counts.event, 1);
  assert.equal(MOST_VIEWED_ARTICLES.length, 4);
