...
export function ProductsToolbar({
  ...
  return (
    <div className="rounded-[3px] bg-white p-4 border border-slate-200/80 shadow-sm space-y-3">
      {/* Top Row: Danh mục + Sort + View Toggle */}
      <div className="flex items-center justify-between gap-3">
        <button
          onClick={onMobileFilterOpen}
          className="lg:hidden inline-flex items-center gap-2 px-3.5 py-2 rounded-[3px] bg-blue-50 text-blue-700 border border-blue-200 text-caption-responsive font-bold shadow-xs shrink-0"
        >
        ...
        <div className="flex items-center gap-3 shrink-0">
          {!isSearchPage && (
            <div className="flex items-center gap-2">
              <span className="text-caption-responsive text-slate-400 font-bold hidden sm:inline text-nowrap">{locale === 'vi' ? 'Sắp xếp:' : 'Sort:'}</span>
              <select
                value={sortBy}
              ...
              >
                <option value="popular">{locale === 'vi' ? 'Nổi bật' : 'Popular'}</option>
