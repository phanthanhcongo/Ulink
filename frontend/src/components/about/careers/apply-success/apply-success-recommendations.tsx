    ...
    <section className="py-10 max-w-5xl mx-auto border-t border-slate-100">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {jobs.map((item) => (
          ...
          >
            <div>
              <span className="text-caption-responsive font-bold text-blue-600 uppercase tracking-wider block mb-1">
                {item.category}
              </span>
              <h3 className="text-card-title text-slate-900 group-hover:text-blue-600 transition-colors mb-3">
                {item.title}
              </h3>
            </div>

            <div className="flex items-center justify-between pt-3 border-t border-slate-100 text-body-regular">
