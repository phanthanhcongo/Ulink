...
export function JobDetailProcess() {
  return (
    <section className="py-6 border-t border-slate-100" id="apply">
      <h2 className="text-[22px] font-extrabold text-slate-900 mb-4 border-l-4 border-blue-600 pl-3">
        Quy trình ứng tuyển
      </h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {processSteps.map((s, idx) => (
          ...
          >
            <span className="text-body-regular font-bold uppercase tracking-wider text-blue-600 mb-1">{s.num}</span>
            <h3 className="text-card-title text-slate-900">{s.title}</h3>
            <p className="mt-1 text-body-regular leading-relaxed text-slate-600">{s.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 rounded-[3px] bg-blue-50 p-5 border border-blue-100 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="block text-body-regular font-bold text-slate-900">
              Liên hệ trực tiếp Phòng Nhân sự ULink
            </span>
            <span className="text-body-regular leading-relaxed text-slate-600">
        ...
        <a
          ...
          className="inline-flex shrink-0 items-center justify-center rounded-[3px] bg-blue-600 px-5 py-2.5 text-body-regular font-semibold text-white shadow-sm hover:bg-blue-700 transition-all"
        >
          Gửi CV qua Email
        </a>
      </div>
