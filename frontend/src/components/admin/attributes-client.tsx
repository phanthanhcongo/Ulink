...
export function AttributesClient({ initialAttributes, error }: AttributesClientProps) {
  ...
  const handleDeleteAttr = (id: number) => {
    setConfirmState({
      isOpen: true,
      title: 'Xóa thuộc tính',
      message: 'Bạn có chắc chắn muốn xóa thuộc tính này? Toàn bộ tùy chọn liên quan sẽ bị xóa.',
      type: 'danger',
      onConfirm: () => {
  ...
  const handleDeleteOpt = (optId: number) => {
    setConfirmState({
      isOpen: true,
      title: 'Xóa tùy chọn',
      message: 'Bạn có chắc chắn muốn xóa tùy chọn này?',
      type: 'danger',
      onConfirm: () => {
        setConfirmState((prev) => ({ ...prev, isOpen: false }));
        startTransition(async () => {
  ...
  return (
    <div className="admin-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-2 admin-panel admin-panel-pad min-h-[400px]">
          {selectedAttr ? (
            <div>
