...
import { ShoppingCart, Check, FileText, Truck, MapPin, Settings2, Minus, Plus, Clock } from 'lucide-react';
...
export default function ProductDetailClient({
  ...
  // Selected size factor (e.g., "3.0 kg" -> 3.0)
  const sizeFactor = useMemo(() => {
    const sizeVal = selections['size'];
    if (!sizeVal) return 1;
    const num = parseFloat(sizeVal);
    return isNaN(num) ? 1 : num;
  }, [selections]);

  // Current active tier unit price
