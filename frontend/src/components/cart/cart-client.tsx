  ...
  return (
    <div className="space-y-10 w-full">
      ...
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 px-4 sm:px-8 lg:px-12 xl:px-16">
        ...
        {/* Step Progress bar */}
        <div className="flex w-full overflow-hidden text-xs sm:text-sm font-semibold rounded-[5px]">
          {/* Step 1: Giỏ hàng */}
        ...
        <div className="grid gap-8 lg:grid-cols-12 pt-2">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {resolvedItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center border border-dashed border-border/80 rounded-[5px] space-y-4">
                <Package className="h-10 w-10 text-muted-foreground/50" />
                <p className="text-sm text-muted-foreground">{t('emptyCart')}</p>
                <Link
                  href="/solutions"
                  className="inline-flex items-center gap-1.5 rounded-[5px] border border-brand px-4 py-2 text-xs font-semibold text-brand hover:bg-brand/5 transition-all"
                >
              ...
              <div className="space-y-4">
                ...
                <div className="hidden md:block overflow-hidden rounded-[5px] border border-slate-200/80 bg-white">
                          ...
                          <td className="px-4 py-4 flex gap-3.5 items-start">
                            {item.slug ? (
                              <Link
                                ...
                                className="relative h-14 w-14 shrink-0 rounded-[5px] border border-slate-200/60 bg-white flex items-center justify-center overflow-hidden hover:opacity-90 transition-opacity block"
                              >
                                {item.hero ? (
                                  <Image
                              ...
                              <div className="relative h-14 w-14 shrink-0 rounded-[5px] border border-slate-200/60 bg-white flex items-center justify-center overflow-hidden">
                                <Package className="h-6 w-6 text-slate-300" />
                              </div>
                            )}
                          ...
                          <td className="px-3 py-4 text-center">
                            <div className="flex items-center justify-center gap-3.5 mx-auto w-fit">
                              <button
                                ...
                                className="p-1 hover:bg-slate-100 rounded-[5px] text-slate-400 hover:text-slate-800 transition-colors"
                              >
                          ...
                          <td className="px-4 py-4 text-right">
                            <div className="flex items-center justify-end gap-3">
                              ...
                              <button
                                ...
                                className="p-2 border border-slate-200 hover:border-slate-300 rounded-[5px] text-slate-400 hover:text-rose-600 hover:bg-slate-50 transition-all flex items-center justify-center shrink-0"
                ...
                <div className="block md:hidden space-y-4">
                  {resolvedItems.map((item, idx) => (
                    <div key={idx} className="rounded-[5px] border border-slate-200/85 bg-white p-4 shadow-sm space-y-4">
                      <div className="flex gap-3">
                        {/* Image */}
                        {item.slug ? (
                          <Link
                            href={`/solutions/${item.slug}`}
                            className="relative h-16 w-16 shrink-0 rounded-[5px] border border-slate-200/60 bg-white flex items-center justify-center overflow-hidden hover:opacity-90 transition-opacity block"
                          >
                            {item.hero ? (
                              <Image
                                src={`${DIRECTUS_URL}/assets/${item.hero}`}
                          ...
                          <div className="relative h-16 w-16 shrink-0 rounded-[5px] border border-slate-200/60 bg-white flex items-center justify-center overflow-hidden">
                            <Package className="h-6 w-6 text-slate-300" />
                          </div>
                        )}
                      ...
                      <div className="flex justify-between items-center border-t border-slate-100 pt-3">
                        <div className="flex items-center gap-3">
                          <button
                            type="button"
                            onClick={() => handleQtyChange(idx, item.quantity - 10)}
                            className="p-1 hover:bg-slate-100 border border-slate-200 rounded-[5px] text-slate-400 hover:text-slate-800 transition-colors"
                          >
                            <Minus className="h-3.5 w-3.5" />
                          </button>
          ...
          <div className="lg:col-span-4 space-y-6">
            {/* Order Summary Panel */}
            <div className="rounded-[5px] border border-slate-200 bg-card p-6 shadow-sm space-y-5">
                ...
                <form onSubmit={handleApplyPromo} className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 rounded-[5px] border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition-all focus:border-brand focus:ring-1 focus:ring-brand font-mono uppercase"
                    />
                    <button
                      type="submit"
                      className="rounded-[5px] bg-[#E0F2FE] hover:bg-[#BAE6FD] text-sky-700 text-xs font-bold px-4 py-2.5 transition-all border border-sky-200"
                    >
                      {t('promoApply')}
                    </button>
                  </div>
      ...
      <div className="w-full bg-card border-y border-slate-200/80 py-10 my-4">
        <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-8 lg:px-16 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <Link
            href="/quick-order"
            className="inline-flex items-center justify-center gap-2 rounded-[5px] bg-[#006AA7] hover:bg-[#005B90] px-6 py-3 text-sm font-bold text-white shadow transition-all shrink-0 w-full md:w-auto"
          >
            {t('rfqCta')}
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-4">
            {suggestedProducts.map((prod, idx) => (
              <div
                key={idx}
                className="rounded-[5px] border border-slate-200/80 p-4 shadow-sm flex flex-col justify-between bg-white hover:border-brand/40 transition-all space-y-4"
              >
                <div className="space-y-3">
                  {/* Fallback/Directus Image */}
                  <Link
                    href={`/solutions/${prod.slug}`}
                    className="aspect-square w-full rounded-[5px] bg-slate-50 border border-slate-200/40 flex items-center justify-center overflow-hidden relative block hover:opacity-90 transition-opacity"
                  >
                    {prod.hero ? (
                      <Image
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-[5px] bg-brand py-2 text-xs font-semibold text-white hover:bg-brand/95 transition-all"
                  >
                    {t('orderNow')}
                  </button>
                  <button
                    type="button"
                    className="p-2 border border-slate-200 text-slate-400 hover:text-slate-600 rounded-[5px] hover:bg-slate-50 transition-all flex items-center justify-center shrink-0"
                    aria-label="Bookmark"
                  >
        ...
        <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-8 lg:px-12 xl:px-16 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto shrink-0 pt-2">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 rounded-[5px] border border-white/60 hover:border-white text-white hover:bg-white/10 px-6 py-3.5 text-sm sm:text-base font-extrabold transition-all text-center"
            >
            ...
            <button
              type="button"
              className="inline-flex items-center justify-center gap-2 rounded-[5px] bg-[#3B82F6] hover:bg-blue-600 text-white px-6 py-3.5 text-sm sm:text-base font-extrabold transition-all text-center shadow-md"
            >
