  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-2.5 rounded-[3px] border border-slate-200 bg-white px-4 py-2.5 text-body-regular font-semibold text-slate-800 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50"
      >
          ...
          {locales.map(({ code, flag: Flag, label }) => (
            <li key={code} role="option" aria-selected={code === locale}>
              <button
                ...
                className={`flex w-full items-center gap-3 px-4 py-2 text-left text-body-regular   transition-colors ${code === locale
