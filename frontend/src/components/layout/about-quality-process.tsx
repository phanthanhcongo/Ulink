  ...
  return (
    <section className="rounded-[3px] bg-background px-6 py-6">
      ...
      <div>
        ...
        <p className="mt-1.5 text-caption-responsive leading-relaxed text-foreground/50">{t('subtitle')}</p>
      </div>
        ...
        {steps.map((step, index) => (
          <Fragment key={step.key}>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              {/* Number */}
              <span className="mt-1.5 text-caption-responsive leading-relaxed text-foreground/60">
                {t(`steps.${step.key}.number`)}
              </span>
              {/* Title */}
              <p className="mt-2 text-body-regular font-bold text-primary">
                {t(`steps.${step.key}.title`)}
              </p>
              {/* Description */}
              <p className="mt-1 text-caption-responsive leading-relaxed text-foreground/40">
            ...
            {index < steps.length - 1 && (
              <div className="hidden items-center justify-center sm:flex">
                <ArrowRight className="h-4 w-4 text-border" strokeWidth={1.5} />
              </div>
            )}
          </Fragment>
        ))}
