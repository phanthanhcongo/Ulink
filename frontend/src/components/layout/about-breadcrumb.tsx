...
const ROUTE_KEYS: { match: (p: string) => boolean; key: string }[] = [
  ...
  {
    match: (p) => p === '/about/sustainability' || p.startsWith('/about/sustainability/'),
    key: 'sustainability'
  },
  {
    match: (p) => p === '/about/capabilities' || p.startsWith('/about/capabilities/'),
    key: 'capabilities'
  },
...
export function AboutBreadcrumb() {
  ...
  return (
    <nav
      ...
      className="flex items-center gap-2 py-4 text-caption-responsive text-muted-foreground"
    >
