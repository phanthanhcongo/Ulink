    ...
    <section className="rounded-[3px] border border-border/40 bg-white px-6 py-6">
      ...
      <div className="mt-5 flex flex-col gap-4">
        {metrics.map((metric) => (
          ...
          >
            <p className="text-caption-responsive leading-relaxed text-foreground/60">{metric.label}</p>
            <p className="text-body-regular font-bold text-brand">{metric.value}</p>
          </div>
        ))}
      </div>

      {/* Footer note */}
      <p className="mt-4 text-caption-responsive text-foreground/40">{t('note')}</p>
    </section>
  );
}
