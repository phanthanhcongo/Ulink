...
export async function AboutHubStats() {
  ...
  const stats = [
    {
      icon: <Warehouse className="h-8 w-8" strokeWidth={1.3} />,
      value: t('stats.areaValue'),
      label: t('stats.areaLabel')
    },
    {
      icon: <Package className="h-8 w-8" strokeWidth={1.3} />,
      value: t('stats.skuValue'),
      label: t('stats.skuLabel')
    },
    {
      icon: <Truck className="h-8 w-8" strokeWidth={1.3} />,
      value: t('stats.timeValue'),
      label: t('stats.timeLabel')
    },
    {
      icon: <ShieldCheck className="h-8 w-8" strokeWidth={1.3} />,
      value: t('stats.isoValue'),
      label: t('stats.isoLabel')
    }
  ];
  ...
  const specs = [
    ...
    {
      icon: <Anchor className="h-[15px] w-[15px]" strokeWidth={1.6} />,
      text: t('location.haiphong')
    },
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_auto]">
      ...
      <div className="flex flex-col gap-5 px-6 py-6 lg:w-[460px] lg:flex-row lg:items-stretch lg:px-0 lg:py-0">
        <div className="flex-1">
          <h3 className="text-caption-responsive font-bold uppercase tracking-wider text-primary">
            {t('location.title')}
          </h3>
          <p className="mt-3 text-caption-responsive leading-relaxed text-foreground/65">{t('location.desc')}</p>

          <ul className="mt-4 flex flex-col gap-2.5">
            {specs.map((spec, i) => (
              <li
                key={i}
                className="flex items-center gap-2 text-caption-responsive font-bold text-foreground/75"
              >
