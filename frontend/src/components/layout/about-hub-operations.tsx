...
export async function AboutHubOperations() {
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_auto_320px]">
      {/* Operations grid */}
      <div>
        <h2 className="mb-5 text-caption-responsive font-bold uppercase tracking-wider text-primary">
          {t('operations.title')}
        </h2>
          ...
          {cards.map((card, i) => (
            <div key={i} className="flex h-full flex-col">
              <div className="flex flex-1 items-start gap-2.5">
                ...
                <div>
                  <p className="text-body-regular font-bold leading-tight text-primary">{card.title}</p>
                  <p className="mt-1.5 text-caption-responsive leading-relaxed text-foreground/60">
                    {card.desc}
                  </p>
                </div>
              </div>
      ...
      {/* ISO standards */}
      <div>
        <h2 className="mb-3 text-caption-responsive font-bold uppercase tracking-wider text-primary">
          {t('iso.title')}
        </h2>
        <p className="text-caption-responsive leading-relaxed text-foreground/65">{t('iso.desc')}</p>
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          {isoBadges.map((badge, i) => (
            <div key={i} className="flex flex-col items-center text-center">
              ...
              <Badge variant="outline" className="mt-2 text-caption-responsive font-bold">
                {badge.code}
              </Badge>
              <p className="mt-1 text-caption-responsive leading-snug text-foreground/60">{badge.label}</p>
            </div>
          ))}
        </div>
      </div>
