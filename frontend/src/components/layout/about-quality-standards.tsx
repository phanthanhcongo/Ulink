  ...
  return (
    <section>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-caption-responsive font-bold uppercase tracking-wider text-primary">{t('title')}</h2>
          <p className="mt-1.5 text-caption-responsive leading-relaxed text-foreground/60">{t('subtitle')}</p>
        </div>
        <a
          href="#"
          className="inline-flex items-center gap-1.5 text-caption-responsive font-medium text-brand hover:underline"
        >
          {t('viewAll')}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        {certs.map((cert) => (
          <Card key={cert.key} className="flex flex-col px-5 py-5">
            {/* Cert icon */}
            ...
            {/* Title */}
            <p className="mt-4 text-body-regular font-bold text-primary">
              {t(`certs.${cert.key}.title`)}
            </p>

            {/* Subtitle */}
            <p className="mt-1 text-caption-responsive leading-relaxed text-foreground/50">
              {t(`certs.${cert.key}.subtitle`)}
            </p>

            {/* Description */}
            <p className="mt-3 flex-1 text-caption-responsive leading-relaxed text-foreground/70">
            ...
            <a
              ...
              className="mt-4 inline-flex items-center gap-1.5 text-caption-responsive font-medium text-brand hover:underline"
            >
      ...
      {showToast && (
        <div className="fixed bottom-5 right-5 z-50 flex items-center gap-2.5 rounded-[3px] border border-amber-200 bg-amber-50 px-4 py-3 text-amber-800 shadow-lg animate-in fade-in slide-in-from-bottom-5 duration-300 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-500/20">
          ...
          <span className="text-body-regular font-semibold">{t('certPending')}</span>
        </div>
      )}
    </section>
  );
