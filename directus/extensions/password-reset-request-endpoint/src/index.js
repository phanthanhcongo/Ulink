...
import { validateEmail, validatePassword, normalizeEmail, EMAIL_RE } from '../../../lib/validation.mjs';
import { BadRequestError, ValidationError, RateLimitedError, AppError } from '../../../lib/errors.mjs';
...
async function recordResetAttempt(email, res, status, code, message) {
  if (!email) return sendError(res, status, code, message);
...
export default {
  ...
  handler(router, context) {
     ...
     * email enumeration.
     */
    router.post('/send', catchRoute(async (req, res) => {
      const { email, purpose: rawPurpose, redirect_path: rawRedirect } = readBody(req);
     ...
     */
    router.post('/peek', catchRoute(async (req, res) => {
     ...
     */
    router.post('/reset', catchRoute(async (req, res) => {
      const { token, password, confirm_password } = readBody(req);
      ...
      if (!token || typeof token !== 'string' || token.length < 32) {
        return sendError(res, 400, 'invalid_token', 'Reset token is missing or invalid.');
      }
      ...
      try {
        ...
        if (!peekedRaw) {
          return sendError(res, 400, 'invalid_token', 'Reset link has expired or already been used. Please request a new one.');
        }
        try {
          peekedEmail = JSON.parse(peekedRaw).email || null;
        } catch {
          await getRedis().del(tokenKey(token)).catch(() => {});
          return sendError(res, 400, 'invalid_token', 'Reset link is invalid.');
        }
      } catch (error) {
      ...
      if (!password || !confirm_password) {
        return sendError(res, 422, 'missing_fields', 'password and confirm_password are required.');
      }
      if (password !== confirm_password) {
      ...
      try {
        ...
        return recordResetAttempt(peekedEmail, res, 422, 'password_policy', err.message);
      }
      ...
      try {
        ...
        return sendError(res, status, error.code ?? 'reset_failed', error.message ?? 'Password reset failed.');
      }
    }));
     ...
     */
    router.post('/password-change/status', catchRoute(async (req, res) => {
     ...
     */
    router.post('/password-change/fail', catchRoute(async (req, res) => {
      const email = normalizeEmail(readBody(req)?.email);
      if (!email || !EMAIL_RE.test(email)) {
        return res.status(200).json({ data: freshCurrentPwdState() });
      }
     ...
     */
    router.post('/password-change/clear', catchRoute(async (req, res) => {
      const email = normalizeEmail(readBody(req)?.email);
      if (!email) {
        return res.status(200).json({ data: { ok: true } });
      }
     ...
     */
    router.post('/user-status', catchRoute(async (req, res) => {
      const email = normalizeEmail(readBody(req)?.email);
