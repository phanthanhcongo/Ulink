...
import { filesToSeedData } from './files_data.mjs';
...
export async function seedInitialContent(helpers, client, geography) {
  ...
  await clearCollection(client, 'product_skus');
  await clearCollection(client, 'products_translations');
  await clearCollection(client, 'products_product_attributes');
  await clearCollection(client, 'products_industries');
  await clearCollection(client, 'products_standards');
  await clearCollection(client, 'products_files');
  ...
  await withDbClient(async (dbClient) => {
    ...
    const filesToSeed = filesToSeedData.map(file => ({
      ...file,
